...
public String getLastIp(String name) {
    return queryString("SELECT last_ip FROM users WHERE name=?", name);
}

@Override
public long getLastLogin(String name) {
    Long v = queryLong("SELECT last_login FROM users WHERE name=?", name);
    return v == null ? 0L : v;
}

@Override
public void updateLoginData(String name, String ip, long time) {
    execute("UPDATE users SET last_ip=?, last_login=? WHERE name=?", ip, time, name);
}
