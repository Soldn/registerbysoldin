...
public class SqlStorage implements Storage {
    ...
    @Override public void init() throws Exception {
        try (Statement st = conn.createStatement()) {
            ...
            st.executeUpdate("CREATE TABLE IF NOT EXISTS ds_links (" +
                    "uuid TEXT PRIMARY KEY, " +
                    "ds_id INTEGER, " +
                    "last_ip TEXT, " +
                    "last_confirm INTEGER" +
                    ")");
            st.executeUpdate("CREATE INDEX IF NOT EXISTS idx_ds_links_ds_id ON ds_links(ds_id)");
        }
    }
    ...
    @Override
    public DsLink getDsLinkByUUID(UUID uuid) {
        try (PreparedStatement ps = conn.prepareStatement("SELECT ds_id,last_ip,last_confirm FROM ds_links WHERE uuid=?")) {
            ps.setString(1, uuid.toString());
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    String lastIp = rs.getString(2);
                    long lastConfirm = rs.getLong(3);
                    return new DsLink(uuid, dsId, lastIp, lastConfirm);
                }
            }
        } catch (Exception e) {
            plugin.getLogger().warning("getDsLinkByUUID error: " + e.getMessage());
        }
        return null;
    }

    @Override
    public DsLink getDsLinkByDsId(long dsId) {
        try (PreparedStatement ps = conn.prepareStatement("SELECT uuid,last_ip,last_confirm FROM ds_links WHERE ds_id=?")) {
            ps.setLong(1, dsId);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    UUID uuid = java.util.UUID.fromString(rs.getString(1));
            ...
            plugin.getLogger().warning("getDsLinkByDsId error: " + e.getMessage());
        }
        return null;
    }

    @Override
    public java.util.List<DsLink> getAllDsLinks() {
        java.util.List<DsLink> list = new java.util.ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement("SELECT uuid,ds_id,last_ip,last_confirm FROM ds_links")) {
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    UUID uuid = java.util.UUID.fromString(rs.getString(1));
                    long dsId = rs.getLong(2);
                    String lastIp = rs.getString(3);
                    long lastConfirm = rs.getLong(4);
                    list.add(new DsLink(uuid, dsId, lastIp, lastConfirm));
                }
            }
        } catch (Exception e) {
            plugin.getLogger().warning("getAllDsLinks error: " + e.getMessage());
        }
        return list;
    }

    @Override
    public void saveOrUpdateDsLink(DsLink link) {
        try (PreparedStatement ps = conn.prepareStatement("REPLACE INTO ds_links(uuid,ds_id,last_ip,last_confirm) VALUES (?,?,?,?)")) {
            ps.setString(1, link.uuid.toString());
            ps.setLong(2, link.dsId);
            ps.setString(3, link.lastIp);
            ps.setLong(4, link.lastConfirm);
            ps.executeUpdate();
        } catch (Exception e) {
            plugin.getLogger().warning("saveOrUpdateDsLink error: " + e.getMessage());
        }
    }

    @Override
    public void deleteDsLinkByUUID(UUID uuid) {
        try (PreparedStatement ps = conn.prepareStatement("DELETE FROM ds_links WHERE uuid=?")) {
            ps.setString(1, uuid.toString());
            ps.executeUpdate();
        } catch (Exception e) {
            plugin.getLogger().warning("deleteDsLinkByUUID error: " + e.getMessage());
        }
    }

    @Override
    public void deleteDsLinkByDsId(long dsId) {
        try (PreparedStatement ps = conn.prepareStatement("DELETE FROM ds_links WHERE ds_id=?")) {
            ps.setLong(1, dsId);
            ps.executeUpdate();
        } catch (Exception e) {
            plugin.getLogger().warning("deleteDsLinkByDsId error: " + e.getMessage());
        }
    }
